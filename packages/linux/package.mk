...
case "${LINUX}" in
  xdarklight)
    PKG_VERSION="0af439236b566a6ccd824320cc1a856f92d483fb" # Meson 8* 20210124
    PKG_SHA256=""
  ...
  *)
    ...
    if [ "${DEVICE}" = "ARMv8" -o "${DEVICE}" = "ARMv7" ]; then

	if [ "${UBOOT_SYSTEM}" = "rk3188" ]; then
		PKG_PATCH_DIRS+=" rk-old"
	else
		PKG_PATCH_DIRS+=" rk"
	fi
...
make_target() {
  ...
  DTC_FLAGS=-@ kernel_make LOADADDR=${SRC_LOADADDR} ${KERNEL_TARGET} ${KERNEL_MAKE_EXTRACMD} modules
